...
function orderAlphabetically(str) {
  const splitOrder = str.split('');
  const sortOrder = splitOrder.sort();
...
function countVowels(str) {
  ...
  const filterStr = splitStr.filter((char) => vowels.includes(char));
...
function isPalindrome(str) {
  ...
  const replaceStr = lowStr.replace(/[\W_]/g, '');
...
function reverseWords(str) {
  const splitStr = str.split(' ');
  const reverseEveryWord = splitStr.map((word) => {
    return word.split('').reverse().join('');
  });
  return reverseEveryWord.join(' ');
}
...
function invertCase(str) {
  const splitStr = str.split('');
  const charSize = splitStr.map((char) =>
    char === char.toUpperCase() ? char.toLowerCase() : char.toUpperCase()
  );
  return charSize.join('');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-zA-Z]/gi, (letter) => rot13[alphabet.indexOf(letter)]);
}
